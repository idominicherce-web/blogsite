...
export default function BlogPostPage({ params, searchParams }: PageProps) {
	return (
		<>
			...
			<PostWrapper>
				{/* Promises are passed down directly so the static wrapper shell can prerender instantly */}
				<Suspense fallback={<BlogPostLoading />}>
					<DeferredPostContent params={params} searchParams={searchParams} />
				</Suspense>
			</PostWrapper>
		</>
	);
...
/**
 * Deferred dynamic post component
 */
async function DeferredPostContent({
	params,
	searchParams,
}: {
	params: Promise<{ slug: string }>;
	...
	// Awaiting dynamic values safely inside the Suspense boundary
	...
	return (
		<main className="max-w-3xl mx-auto p-4 sm:p-6 space-y-12 md:space-y-16 relative z-10 pt-4 md:pt-16">
			<nav className="flex flex-wrap items-center justify-between gap-2 pl-14 sm:pl-0 relative z-50">
				{isAdmin ? (
					<span className="inline-flex items-center whitespace-nowrap text-[9px] sm:text-[10px] font-sans font-extrabold uppercase tracking-[0.15em] bg-emerald-950/40 border border-emerald-800/50 px-2 py-1 sm:py-0.5 rounded-xs animate-pulse text-emerald-400">
						🛡️ Admin Mode Active
					</span>
				) : (
				...
				<article className="relative mx-1 sm:mx-3 px-4 sm:px-16 py-10 sm:py-12 bg-linear-to-b from-[#fff9e8] via-[#fdf3d6] to-[#f9e7c9] text-zinc-900 border-x-8 sm:border-x-12 border-amber-900/10 z-10 rounded-sm shadow-[0_25px_60px_rgba(0,0,0,0.5),0_40px_100px_rgba(234,88,12,0.15),inset_0_-20px_80px_rgba(217,119,6,0.1)]">
					...
					<footer className="mt-16 pt-8 border-t border-amber-900/20 space-y-6">
						<div className="flex justify-start pl-2 sm:pl-4">
							...
							>
								<TossCoinContainer postId={post.id} initialCoins={post.coins} />
							</Suspense>
						</div>
			...
			<div className="pt-2 relative z-20">
				...
				>
					<CommentSection postId={post.id} postSlug={slug} isAdmin={isAdmin} />
				</Suspense>
			</div>
		</main>
	);
